...
bool InjectDll(const DWORD pid, const char* oldString, const char* newString)
{
	...
	setParamsFunction(oldString, newString);
...
int main()
{
	const std::wstring processName = L"Injection.exe";
	...
	InjectDll(processId, oldString.c_str(), newString.c_str());
